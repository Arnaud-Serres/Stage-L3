...
def tftft(x, wintype, win_duration, sigma, a=1, Fs=1):
    ...
    V = np.zeros((math.floor(L / 2) + 1, N), dtype=complex)
    ...
    for n in range(N):
        ...
        # h_t = np.zeros(L)
        # if wintype == "gauss":
        #     h_t[range(winlen_t)] = gaussian(winlen_t, winlen_t / 10)
        # elif wintype == "hann":
        #     h_t[range(winlen_t)] = hann(winlen_t)
        # else:
        #     h_t[range(winlen_t)] = boxcar(winlen_t)
        # h_t *= np.sqrt(sigma[n])

        # Center the window at the appropriate location
        # h_t = np.roll(h_t, t - math.floor(winlen_t / 2))
